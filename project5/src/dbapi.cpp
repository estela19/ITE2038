...
int DBManager::db_find(int table_id, int64_t key, char* ret_val, int trx_id) {
    Page root;
    Page header(table_id, 0);
    if(header.page->header.root_pnum != 0){
        root.pnum = header.page->header.root_pnum;
    ...
    if(bpt->Find(&root, &tmp, key) != 0){
        ...
        LockManager::get().lock_acquire(table_id, key, trx_id, lockmode::SHARED);
...
int DBManager::db_update(int table_id, int64_t key, char* values, int trx_id){
    Page root;
    Page header(table_id, 0);
    if(header.page->header.root_pnum != 0){
        root.pnum = header.page->header.root_pnum;
    ...
    if(bpt->Find(&root, &tmp, key) != 0){
        ...
        LockManager::get().lock_acquire(table_id, key, trx_id, lockmode::EXCLUSIVE);
        bpt->Update(&root, values, key);
        return 0;
    }
}
